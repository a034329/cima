...
def build_prompt(nombre: str, anclas: dict, tipo_val: str = "PER",
    ...
    system = (
        ...
        f"REGLA — FRESCURA DEL DATO: cuando uses búsqueda web para anclas "
        f"críticas (EPS NTM consenso, múltiplo objetivo del sector, target "
        f"medio de analistas), prioriza por FECHA del dato. Las estimaciones se "
        f"revisan trimestralmente post-earnings; un dato de hace 3 meses puede "
        f"estar obsoleto frente a uno de hace 2 semanas. Si varias fuentes "
        f"divergen, explora si la divergencia es por FECHA (usar la más "
        f"reciente) o por METODOLOGÍA (GAAP vs non-GAAP, número de analistas) "
        f"— declara la diferencia en la razón y tira conservador. Las anclas "
        f"que te da Cima (EPS_actual, PER_hist, precio_obj_consenso) son "
        f"snapshot de Cima; si la web te da un dato más fresco para la misma "
        f"métrica, MENCIÓNALO en la razón ('consenso EPS NTM actualizado a X "
        f"según fuente Y de fecha Z, vs ancla de Cima W') y úsalo.\n"
